...
Club Anti-Zapret - это набор PowerShell-скриптов, предназначенный для автоматического и скрытного запуска обхода ограничений в компьютерных клубах. Основной скрипт использует инструменты `zapret-discord-youtube` версии 1.8.5 для обеспечения беспрепятственного доступа к заблокированным сервисам, включая Discord, YouTube и другие, в условиях, когда соединение с этими сервисами заблокировано или ограничено.
...
- **Улучшенное завершение**: После завершения работы приложения скрипт завершает процессы обхода, но не удаляет временные файлы (они очищаются системой автоматически)
...
- **Улучшенная проверка инициализации**: Более точная проверка готовности обхода к работе
- **Улучшенная очистка процессов**: Более тщательное завершение процессов обхода и Discord
...
- `zapret-discord-youtube` - инструменты обхода ограничений (версия 1.8.5)
...
- **CatSema** - скрипт DiscordBypass и сопутствующие инструменты
